...
async function getConnectedMentorIds(
...
export async function getQuizzesForDashboard(
  studentId: string
): Promise<{ data: DashboardQuiz[] | null; error: string | null }> {
  ...
  const formattedQuizzes = quizzes.map((q) => {
    ...
    const profile = Array.isArray(mentorProfile)
      ? mentorProfile[0]
      : mentorProfile;
    return {
      id: q.id,
...
export async function getAnnouncementsForDashboard(
  studentId: string
): Promise<{ data: DashboardAnnouncement[] | null; error: string | null }> {
  const cookieStore = cookies();
  const supabase = createClient(cookieStore);
  const mentorIds = await getConnectedMentorIds(supabase, studentId);
