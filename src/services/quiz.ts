...
export async function getMentorQuizzesForStudent(
  studentId: string
): Promise<{ data: MentorQuiz[] | null; error: string | null }> {
  ...
  if (quizError) {
    ...
    return {
      data: null,
      error: 'Failed to fetch quizzes from your mentors.',
    };
  }

  const formattedQuizzes =
    quizzes?.map((q) => {
      const mentorProfile = q.mentor as Profile | null;
      return {
        id: q.id,
        title: q.title,
        topic: q.topic,
        mentor_name: mentorProfile?.full_name || 'N/A',
      };
    }) || [];
...
export async function getQuizForStudent(
  quizId: number,
  studentId: string
): Promise<{ data: QuizWithQuestions | null; error: string | null }> {
  const cookieStore = cookies();
  const supabase = createServerClient(cookieStore);
